...
>
> **Current software direction:** a pnpm/Turborepo workspace contains the Astro website, PostgreSQL-backed Node API, Vite/React console and a separate Better Auth service. The console manages submitted interests, contact details, follow-up status and staff notes. Email/password accounts have admin and staff roles; admins manage accounts and revoke access. App-local `.env` files hold configuration; seeded credentials live in `apps/backend/auth/.env` and the workspace-root `.env` is not loaded. Railway infrastructure is authored in `.railway/railway.ts` for `thepit / development`, preserving the existing database and volume. Email delivery requires Resend configuration on the API. Membership registration, payments, attendance and broader automation remain future planning work. Historical maktab, bought-platform and September-launch tasks below are retained for context and are not current commitments.
>
